...
const db = mysql.createConnection(
  {
    host: "localhost",
    // Tried hiding password using dotenv, but could not get it to work
    ...
    .then((response) => {
      ...
      db.query(sql, response.departmentName, (err, result) => {
        if (err) {
          ...
          console.log(
            `Department ${response.departmentName} added to database`
          );
          displayOptions();
        }
      });
              ...
              db.query(
                ...
                (err, result) => {
                  if (err) {
                    ...
                    console.log(
                      `Added the employee, ${response.firstName} ${response.lastName} to the database.`
                    );
                    displayOptions();
                  }
                }
